...
export default function LanguageSelector({ className = '' }: LanguageSelectorProps) {
  ...
  if (isLoading) {
    return (
      <View
        className={`${className} bg-secondary-100 rounded-xl p-4 items-center py-8`}
      >
        <ActivityIndicator size="small" color="#18C06A" />
      </View>
    );
  }
  return (
    <View className={`${className}`}>
      <View className="bg-secondary-100 rounded-xl p-4">
