...
import { deleteWallet, updateWallet } from "@/lib/appwrite";
...
const WalletUpdate = () => {
  ...
  const handleUpdateWallet = async () => {
    ...
    if (!formData.name.trim()) {
      Alert.alert("Completo los campos", "El nombre de la cartera es requerido");
      return;
    }
    ...
    try {
      ...
      if (updatedWallet) {
        ...
        Alert.alert("Algo salio mal", "No se pudo actualizar la cartera");
      }
    } catch (error) {
      console.error("Error updating wallet:", error);
      Alert.alert("Algo salio mal", "Ocurrió un error al actualizar la cartera");
    } finally {
      setIsLoading(false);
    }
  };
      ...
      [
        ...
        },
        {
          ...
          onPress: async () => {
            setIsDeleting(true);
            try {
              const success = await deleteWallet(walletToEdit.id);
              ...
              if (success) {
                ...
                Alert.alert(
                  "Algo salio mal",
                  "No se pudo eliminar la cartera. Asegúrate de que no tenga transacciones."
                );
              }
            } catch (error) {
              console.error("Error deleting wallet:", error);
              Alert.alert("Algo salio mal", "Ocurrió un error al eliminar la cartera");
            } finally {
              setIsDeleting(false);
            }
          },
